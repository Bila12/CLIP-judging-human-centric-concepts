...
ex = Experiment(name="create_feature_dataset"  )  # Create a sacred experiment
...
def main(_run):
    ...
    file_id = []
    for filename in os.listdir("./poster_after_CLIP/posters"):
...
############################################################################################################################################################################    
    ...
    dataset = PlotsterDataset(text_file = "list_clear_id_test.txt", transform1 = clip_transform, transform2 = bit_transform, transform3 = RN_transform)
    ...
    for i_batch, sample in enumerate(train_dataloader):
        ...
        #print(clip_im_features.squeeze(dim = 0).size())
        #print(clip_im_features.size())
        '''
        ...
        '''
        if not os.path.exists('./features_after_CLIP/test_features/RN_posters'):
            os.makedirs('./features_after_CLIP/test_features/RN_posters')
            os.makedirs('./features_after_CLIP/test_features/BiT_posters')
            os.makedirs('./features_after_CLIP/test_features/CLIP_posters')
            os.makedirs('./features_after_CLIP/test_features/CLIP-T_titles')
            os.makedirs('./features_after_CLIP/test_features/CLIP-T_plots')
            os.makedirs('./features_after_CLIP/test_features/Bert-small_plots')
            os.makedirs('./features_after_CLIP/test_features/Bert-small_titles')
            os.makedirs('./features_after_CLIP/test_features/Bert-large_plots')
            os.makedirs('./features_after_CLIP/test_features/Bert-large_titles')
            
        torch.save(RN_im_features.squeeze(dim = 0).detach(),'./features_after_CLIP/test_features/RN_posters/%s.pt' % (idx[0]) ) 
        torch.save(bit_im_features.squeeze(dim = 0).detach(),'./features_after_CLIP/test_features/BiT_posters/%s.pt' % (idx[0]) ) 
        torch.save(clip_im_features.squeeze(dim = 0).detach(),'./features_after_CLIP/test_features/CLIP_posters/%s.pt' % (idx[0]) ) 
        
        torch.save(clip_title_features.squeeze(dim = 0).detach(),'./features_after_CLIP/test_features/CLIP-T_titles/%s.pt' % (idx[0]) ) 
        torch.save(clip_plot_features.squeeze(dim = 0).detach(),'./features_after_CLIP/test_features/CLIP-T_plots/%s.pt' % (idx[0]) ) 
        torch.save(BL_plot_features.squeeze(dim = 0).detach(),'./features_after_CLIP/test_features/Bert-large_plots/%s.pt' % (idx[0]) ) 
        torch.save(BL_title_features.squeeze(dim = 0).detach(),'./features_after_CLIP/test_features/Bert-large_titles/%s.pt' % (idx[0]) ) 
        torch.save(BS_plot_features.squeeze(dim = 0).detach(),'./features_after_CLIP/test_features/Bert-small_plots/%s.pt' % (idx[0]) ) 
        torch.save(BS_title_features.squeeze(dim = 0).detach(),'./features_after_CLIP/test_features/Bert-small_titles/%s.pt' % (idx[0]) )
        ''' 
    ...
    for i_batch, sample in enumerate(valid_dataloader):
        ...
        if not os.path.exists('./features/valid_features/RN_posters_norm'):
            os.makedirs('./features/valid_features/RN_posters_norm')
        torch.save(RN_im_features.squeeze(dim = 0).detach(),'./features/valid_features/RN_posters_norm/%s.pt' % (idx[0]) ) 
    ...
    for i_batch, sample in enumerate(test_dataloader):
        ...
        if not os.path.exists('./features/test_features/RN_posters_norm'):
            os.makedirs('./features/test_features/RN_posters_norm')
        torch.save(RN_im_features.squeeze(dim = 0).detach(),'./features/test_features/RN_posters_norm/%s.pt' % (idx[0]) )  
