...
for language_model in ["CLIP-T","Bert-large","Bert-small"]:
    for vision_model in ["CLIP","BiT", "RN50"]:
        if language_model is not None or vision_model is not None:
            ...
            for i in range(5):
                ...
                ex = Experiment(name="%s%s%s%s%sclassification_features" % (Nonestring(vision_model), Nonestring(language_model), Nonestring(isImUsed), Nonestring(text_to_encode), Nonestring(filteredString) ) )  # Create a sacred experiment
                ...
                ############################################################################################################################################################################    
                    ...
                    ############################################################################################################################################################################
                        for i_batch, sample in enumerate(dataloader):
                            ...
                            _run.log_scalar("Training_loss", loss.cpu().detach().numpy().tolist(), step)
                            ...
                            _run.log_scalar("Training_w_acc", w_acc, step)  
                    ...
                    ############################################################################################################################################################################            
                            if len(dataloader)-1==step:
                                ...
                                for i_batch, sample in enumerate(valid_dataloader):
                                    ...
                                    _run.log_scalar("valid_loss", loss.cpu().detach().numpy().tolist(), val_step)
                                    
                                    w_acc = ((torch.mul(1-genre_input, 1-genre_predicted).sum().item()/batch_size)/((1-genre_input).sum().item()/batch_size)+(torch.mul(genre_input, genre_predicted).sum().item()/batch_size)/(genre_input.sum().item()/batch_size))/2
                                    
                                    _run.log_scalar("Valid_w_acc", w_acc, val_step)
                    ...
                    ############################################################################################################################################################################
                                ...
                                if epoch == max_epoch-1 :
                                    ...
                                    for i_batch, sample in enumerate(test_dataloader):
                                        ...
                                        _run.log_scalar("Test_loss", loss.cpu().detach().numpy().tolist(), val_step)
                                        ...
                                        _run.log_scalar("Test_w_acc", w_acc, val_step )
